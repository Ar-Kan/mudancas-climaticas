...
p <- ggplot(df, aes(x = Year, y = Meteorological.year, fill = Meteorological.year)) +
  geom_bar(stat = "identity", color = ifelse(df$Meteorological.year < 0, "gray60", NA)) +
...
p <- ggplot(dfw, aes(x = Year, y = Meteorological.year, fill = Meteorological.year)) +
  geom_bar(stat = "identity", color = ifelse(dfw$Meteorological.year <= 0.001, "gray60", NA)) +
  scale_fill_viridis(option = "H") +
...
ggplot(data = dfl, aes(x = Value)) +
  ...
  labs(x = "Variação em °C",
       y = "Frequência") +
  dark_theme +
  theme(strip.background = element_rect(fill = "gray15", colour = "#191919",
...
Com 95% de confiança, rejeitam-se as **TODAS** as hipóteses nulas, portanto:
